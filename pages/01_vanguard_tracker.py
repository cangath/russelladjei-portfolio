...
with st.sidebar:
    ...
    st.info(
        "💡 Auto-scraping runs daily via GitHub Actions\n\n"
    ...
    yield_val = st.number_input(
        "7-Day SEC Yield (%)", min_value=0.0, max_value=10.0, step=0.01
    )
...
with tab1:
    ...
    if data:
        ...
        st.info(
            "📊 No data yet. Add yields manually or wait for "
            "GitHub Actions auto-scrape (runs daily at 9 AM EST)."
        )
    ...
    if tickers:
        ...
        if selected:
            ...
            if rows:
                ...
                pivot_df = df_hist.pivot(
                    index="Date", columns="Ticker", values="Yield"
                )
                ...
                for ticker in selected:
                    ...
                    if not ticker_data.empty:
                        ...
                        with col1:
                            st.metric(
                                f"{ticker} Latest",
                                f"{ticker_data['Yield'].iloc[-1]:.3f}%",
                            )
                        with col2:
                            st.metric(
                                f"{ticker} Avg",
                                f"{ticker_data['Yield'].mean():.3f}%",
                            )
                        with col3:
                            st.metric(
                                f"{ticker} Max",
                                f"{ticker_data['Yield'].max():.3f}%",
                            )
                        with col4:
                            st.metric(
                                f"{ticker} Min",
                                f"{ticker_data['Yield'].min():.3f}%",
                            )
    else:
...
The 7-day SEC yield is an **annualized yield** calculated from a fund's net investment
income over the past 7 days. It's standardized by the SEC.
...
### Data Storage
All data is stored in the GitHub repository at `data/vanguard_yields.json`.
...
Vanguard updates yields on market days (Mon–Fri). On weekends, the dashboard
displays the most recent market day's data.
"""
    )
